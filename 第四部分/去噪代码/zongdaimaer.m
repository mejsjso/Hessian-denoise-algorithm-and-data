...
% 读取图像
% 请确保路径正确
img1 = imread('C:\Users\HUAWEI\Desktop\数值方法大作业资料包\第四部分\分辨率分析测试图\原始图像\分辨率分析测试图(f=0.1,-60°).png');
img = imread('C:\Users\HUAWEI\Desktop\数值方法大作业资料包\第四部分\分辨率分析测试图\处理图像\分辨率分析测试处理图(f=0.1,-60°).png');
...
function ssim_val = calculateSSIM(original, processed)
    ...
    C1 = (K(1)*L)^2;
    C2 = (K(2)*L)^2;
    mu1 = filter2(fspecial('gaussian', 8, 1.5), original, 'valid');
    mu2 = filter2(fspecial('gaussian', 8, 1.5), processed, 'valid');
    sigma1_sq = filter2(fspecial('gaussian', 8, 1.5), original.^2, 'valid') - mu1.^2;
    sigma2_sq = filter2(fspecial('gaussian', 8, 1.5), processed.^2, 'valid') - mu2.^2;
    sigma12 = filter2(fspecial('gaussian', 8, 1.5), original.*processed, 'valid') - mu1.*mu2;
